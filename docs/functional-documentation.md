...
- **Total damage distribution**: a histogram of the distribution of total damage dealt over the whole sequence (0 up to `boxesInitial - 1`), with every outcome that destroys the target grouped into a single aggregated bucket labelled `"N+"` (e.g. `"5+"` for a 5-box target) — since a destroyed target's exact overkill isn't tracked beyond "it reached or exceeded its box count". A **Normal / At least** switch above the histogram toggles between two readings of the SAME underlying distribution: *Normal* shows the chance of dealing precisely that much damage (the default); *At least* shows the cumulative chance of dealing that much damage OR MORE (every bucket relabelled `"N+"`, each one strictly larger than the bucket above it) — more useful for "will this at least knock the target down to N boxes" questions than reading off a run of individually small bars.
