...
- **Random ROF for ranged weapons** (a die roll at the top of the sequence decides how many shots that attack actually fires) — **M/L**. A genuine new engine feature, but a simpler one than Critical Shred was: the shot count is resolved ONCE, before any dice for that attack are rolled, rather than recursively depending on outcomes - still needs a new branching dimension threaded through the backward induction and forward simulation, just without Critical Shred's self-referential recursion.
