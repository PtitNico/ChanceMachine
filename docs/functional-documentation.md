...
- **Special rules**: **Tough** / **Tough Steady** (mutually exclusive — activating one deactivates the other; see "Modeled rules" for the difference between them), **Shield** (a flat ARM bonus toggle, 0 to 10), **Unyielding** (+2 ARM against melee attacks only), **Carapace** (+4 ARM against ranged attacks only), **Rapid Healing** (heals d3 boxes after a damaging, non-destroying hit — see "Target capabilities" below).
- **Custom effects**: a repeatable list for spell-granted (or non-spell, e.g. feat) stat bonuses or capabilities that aren't worth naming individually (there are far too many to enumerate) — see "Custom effects" below.
...
- **General**: Jump the Shark — applies **to both** the to-hit roll and the damage roll (a single button for both, rather than a separate setting per roll) —, Blessed (ignores every Stat-type bonus flagged **Spell** on the target — see "Custom effects" below).
...
- **Blessed**: this attack ignores every **Stat**-type bonus flagged **Spell** on the target (both DEF and ARM) — a non-spell Stat-type bonus (a feat, a non-spell aura) is never Blessed-ignorable, though it's still affected by Armor Piercing like any other ARM bonus. Shield, Unyielding, Carapace, and any **dispellable effect** grant are all unaffected too, since those aren't Stat-type bonuses.
...
- **Dispel**: removes every custom effect currently flagged **Dispellable** (see "Custom effects" below) from the target, for the rest of the sequence. A **dispellable effect** entry (added via "Add dispellable effect") is always Dispellable, so any Tough/Unyielding it grants goes away too; a **stat spell** entry only goes away if its own Dispellable toggle is still on. Innate capabilities toggled directly in "Special rules" are never affected — only entries from Custom effects can be dispelled. Like the other persistent effects, once triggered it applies to every later attack in the sequence, never the attack that triggered it.
...
Warmachine/Hordes has far too many spells (and feats, and non-spell auras) to list individually, and each one either grants a flat stat bonus or a named capability for the rest of the game. Rather than trying to enumerate them, the Target profile pop-up's **Custom effects** section offers two buttons for building a repeatable list:

- **+ Add dispellable effect**: a granted rule — **Tough** or **Unyielding** — applied to the target exactly as if it had been toggled directly in the "Special rules" section above, with the same conditions (Unyielding still only applies against melee attacks). Only these two are offered here (Tough Steady, Shield, and Carapace are almost always innate model rules rather than spell-granted in practice, so they're left off this list to keep it short). Only one rule per entry; a spell granting several needs one entry each. Always removed by an attack's **Dispel** effect — there's no checkbox, since an upkeep effect granting a rule is always dispellable in this model.
- **+ Add stat spell**: a flat **DEF or ARM bonus** (0 to 10), applied unconditionally (regardless of attack type), with two independent toggles:
  - **Spell**: on means an attack's **Blessed** effect ignores this bonus entirely (the common case for an actual spell buff); off means it's a non-spell source (a feat, a non-spell aura) Blessed never ignores — though it's still affected by Armor Piercing like any other ARM bonus.
  - **Dispellable**: on means an attack's **Dispel** effect removes this bonus for the rest of the sequence (an Upkeep/animus spell); off means it's permanent for the whole sequence (a one-shot spell effect, or a feat/aura that was never removable in the first place).

Both a **name** (free text, purely for the player's own reference — it doesn't affect the calculation) and the toggles above combine into four common cases: a one-shot spell bonus (Spell on, Dispellable off), an upkeep spell bonus (Spell on, Dispellable on), an upkeep spell that grants a rule (always via "Add dispellable effect"), and a non-spell bonus like a feat (Spell off, Dispellable off).

As many entries as needed can be added at once (a caster can have several buffs active on the same model simultaneously) — each is removed individually with its own **✕** button.
...
- **Custom effects are the one deliberate exception, by necessity**: since Warmachine/Hordes has far too many spells (and feats/non-spell auras) to name individually, the Target profile's Custom effects section is a small generic system (name + Stat-or-Rule + Spell/Dispellable tags) rather than a named list — the only place in the app where the player enters a raw stat bonus instead of picking a named, pre-validated effect. Reusing the same "Special rules" toggles for the dispellable-effect case keeps attack-type conditions (Unyielding/Carapace) and Tough/Tough Steady exclusivity correct for free, rather than reimplementing them a second time for the spell path. Blessed and Dispel then read the Spell/Dispellable/Stat-or-Rule tags directly, so this generic system pays off across both the Custom effects section itself and the two attack effects that interact with it. The Spell tag also closes a gap that used to be unmodelable: a non-spell stat bonus (a feat, a non-spell aura) Blessed shouldn't ignore, but Dispel still shouldn't remove.
