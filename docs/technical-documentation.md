...
`AppMenu` itself owns none of the three actions - it only emits `reset`/`openAbout`/`openFeedback` outputs and closes itself, mirroring how `TargetPanel`'s cog button emits `openProfile` rather than owning `TargetProfileDialog` (see "Component split" above):
