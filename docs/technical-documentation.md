...
- A Tough roll is retried every time damage (after any Focus/Fury mitigation) would be lethal (no "once per turn" limit in this model — a documented simplification in the code) - **unless the target is currently Knocked Down/Stationary**, in which case plain Tough can't be attempted at all (the real tabletop rule) and the target is simply destroyed; **Tough Steady** (`SequenceTarget.toughSteady`) is immune to that negation - see `damageBranches`.
...
- Every boolean flag (`knockedDown`, `stationary`, `shadowbind`, `blind`, `paralyzed`, `flare`, `weaken`, `dispelled`) is **idempotent** — re-triggering it has no further effect, in line with the "an effect only applies once unless stated otherwise" rule. `dispelled` is the odd one out semantically (it strips bonuses rather than adding a penalty) but mechanically it's just another sticky flag, same `applyStatEffect` switch case as the others.
...
- Critically, **`baseArm`/`baseDef` themselves are never touched by any of this** - they stay the target's raw printed stats, because Armor Piercing explicitly ignores "any ARM buff or debuff" (see `attack-model.ts`) and must keep using the true base value regardless of how many buffs are active or Dispelled. This is why every one of these bonuses is resolved as a separate addition in `profileFor` rather than folded into `target.arm` before it ever reaches the engine.
...
```
`damageBranches` picks `debuffState.dispelled ? tough.hasToughPostDispel : tough.hasTough` (and the Tough Steady equivalent) before applying the existing Knocked Down/Stationary negation rule - Dispel and the Knockdown-negation rule are independent conditions that both have to allow a Tough attempt for it to actually happen. `hasAnyTough` (used by `computeReachableDebuffStates`'s over-approximation, and by whether the fail chance is computed at all) stays `hasTough || hasToughSteady` without needing the post-Dispel variants folded in: by construction, `hasToughPostDispel`/`hasToughSteadyPostDispel` can only be true if the corresponding pre-Dispel flag is already true (post-Dispel is strictly the innate-only subset of pre-Dispel, per the invariant above), so they never add a state pre-Dispel wouldn't already cover.
...
Every child component that owns its own popup (`EffectsDialog`, `TargetProfileDialog`, `DetailsDialog`) is self-contained: it holds its own `<dialog>` via `@ViewChild` and exposes `open()`/`close()` methods directly, rather than the orchestrator tracking "which dialog/row is open". The orchestrator's template calls these methods through **template reference variables** (`#effectsDialogRef`, `#targetProfileDialogRef`, `#detailsDialogRef`) rather than `@ViewChild` in the orchestrator's own class - Angular resolves a template ref variable anywhere in the same template, including from a sibling element's event binding, so no `@ViewChild` boilerplate is needed there at all.
...
**`TriggerEffectRow`: a FIXED set rather than a dynamic list.** Unlike an earlier version that managed persistent effects as an extensible `WritableSignal<StatEffectRow[]>` list ("+ Add effect" button, removal by `id`), the current version gives each row a `triggerEffects` array of fixed, known-in-advance size - one `TriggerEffectRow` per key of `TRIGGER_EFFECT_KEYS` (`'armorPiercing' | 'decapitation' | StatEffectType`, 12 entries: `armorPiercing`, `decapitation`, the 9 persistent debuffs, and `dispel`), created once by `createTriggerEffects()` and never recreated during the row's lifetime:
```ts
...
- "Special rules": Tough / Tough Steady (mutually exclusive via `toggleToughKind`, see `TargetState` above), Shield (amount select appears once active, same pattern as `-X ARM`), Unyielding, Carapace — all five toggles share one `.toggle-group` section.
...
`html { overscroll-behavior-y: none; }` (`src/styles.css`) disables the browser's pull-to-refresh gesture in the installed PWA: on Android, that gesture could trigger a viewport/`dvh` recalculation glitch in some WebView versions that left the fixed layout mis-sized after the reload, requiring a scroll to reach content that should already be visible - and there's nothing server-side to actually refresh in this app anyway (everything recomputes instantly and works offline).
