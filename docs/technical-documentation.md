...
Every child component that owns its own popup (`EffectsDialog`, `TargetProfileDialog`, `DetailsDialog`, `AboutDialog`, `ChangelogDialog`, `FeedbackDialog`) is self-contained: it holds its own `<dialog>` via `@ViewChild` and exposes `open()`/`close()` methods directly, rather than the orchestrator tracking "which dialog/row is open". The orchestrator's template calls these methods through **template reference variables** (`#effectsDialogRef`, `#targetProfileDialogRef`, `#detailsDialogRef`, `#aboutDialogRef`, `#changelogDialogRef`, `#feedbackDialogRef`) rather than `@ViewChild` in the orchestrator's own class - Angular resolves a template ref variable anywhere in the same template, including from a sibling element's event binding, so no `@ViewChild` boilerplate is needed there at all. `AppMenu` follows the same "trigger elsewhere, dialog lives at the orchestrator level" shape as `TargetPanel`'s cog button: it doesn't hold any of these dialogs itself, it just emits `openAbout`/`openChangelog`/`openFeedback` outputs that the orchestrator wires straight to those template ref variables.

**Shared CSS via multiple `styleUrls`, not a single global stylesheet.** Angular components normally get their own scoped CSS file, but many classes here (`.mini-field`, `.icon-btn`, `.dialog__header`, `.toggle-btn`, `.effect-tag`, ...) are used by several sibling components with identical markup. Rather than duplicating the rules by hand (risking drift) or making them globally unscoped (risking accidental collisions elsewhere in the app), each consuming component lists the relevant partial file(s) from `shared/` in its own `styleUrls` array alongside its own file. Angular bundles and scopes each partial **separately per consuming component** (each gets its own `[_ngcontent-xxx]` attribute stamped onto the rules), so a shared file can be included by five different components and each one only ever matches elements actually rendered by that component - a `.attack-row` selector inside `shared/scrollable-row.css`, for instance, simply never matches anything inside `TargetPanel`'s scoped copy of that file, since `TargetPanel`'s template has no such element.
...
Pure TypeScript (no Angular), imported by the orchestrator, `AttackRowComponent`, and `EffectsDialog`. Each row (`AttackRow`) is an object whose **every field is itself a signal** - see the "why" below. No attacker name or editable label: the label shown in the results (`Attack N`) is generated from the row's position. Base fields: `type`, `stat`, `diceCount`, `forceAutoHit`, `pow`, `damageDiceCount`. Effect fields:
- General: `jumpTheShark` (boolean) - a single signal for both the attack and damage rolls; `blessed` (boolean) - ignores every Stat-type spell bonus on the target, resolved by `sequence.ts`'s `profileFor` rather than here (see "Target capabilities" below).
...
- Damage roll: `discardDamageLowest`/`discardDamageHighest` (independent booleans), `rerollDamage`, `trash`, `shatter`, `chainWeapon` (booleans) - `chainWeapon` ignores the target's Shield ARM bonus specifically, same resolution point as `blessed`.
- Crit only: `brutalDamage`, `criticalShred` (booleans) - both placed here rather than in `triggerEffects` since neither has an "on hit" variant.
- Effects triggerable on a hit/critical hit: `triggerEffects: TriggerEffectRow[]` - see below.

**`TriggerEffectRow`: a FIXED set rather than a dynamic list.** Unlike an earlier version that managed persistent effects as an extensible `WritableSignal<StatEffectRow[]>` list ("+ Add effect" button, removal by `id`), the current version gives each row a `triggerEffects` array of fixed, known-in-advance size - one `TriggerEffectRow` per key of `TRIGGER_EFFECT_KEYS` (`'armorPiercing' | 'decapitation' | StatEffectType`, 13 entries: `armorPiercing`, `decapitation`, the 9 persistent debuffs, `dispel`, and `grievousWounds`), created once by `createTriggerEffects()` and never recreated during the row's lifetime:
```ts
...
- `toSequencedAttack(row, index)`: a named function (not an inline `.map()` callback) with an explicit `SequencedAttack` return type annotation, projecting one `AttackRow` into the plain object shape the engine expects — generates the `Attack ${index+1}` label, converts `pow` via `resolvePow`, applies `row.jumpTheShark()` to both `modifiers.treatOnesAsSixes` and `damageModifiers.treatOnesAsSixes`, assembles `effects`/`statEffects`. The orchestrator's `sequencedAttacks` computed is then just `this.rows().map((row, i) => toSequencedAttack(row, i))`.
...
- `resetEffects(row)`: resets every effect field on the row to its default — the "base" fields (type, stat, dice, POW) are deliberately left untouched.
...
Implementation: the native HTML **`<dialog>`** element (no modal/CDK library). Each dialog component owns its `<dialog>` via `@ViewChild` and exposes `open()`/`close()`:
...
- `closeOnBackdropClick(event, dialog)` (duplicated verbatim in each dialog component, small enough not to warrant a shared helper) closes the pop-up on a click outside its content: for a `<dialog>` opened in modal mode, a click on the `::backdrop` bubbles up a `click` event whose `target` is the `dialog` element itself (not a child) — this property lets us distinguish "click on the backdrop" from "click inside the content" without `stopPropagation()`. The Escape key closes the pop-up natively, with no extra code.

**Content of the Effects pop-up**, organized as "toggle" buttons (`.toggle-btn` / `.toggle-btn--active`, defined in `shared/dialog-sections.css`) grouped into sections (see the functional documentation for the details of each effect):
- Auto-hit (a single button, at the top, outside any section).
- "General": Jump the Shark (a single button for `row.jumpTheShark`, which drives both the to-hit roll and the damage roll), Blessed (`row.blessed`, resolved by `sequence.ts`'s `profileFor` - see "Target capabilities").
...
- "Damage": Discard lowest, Discard highest, Reroll, Trash, Shatter, Chain Weapon (`row.chainWeapon`, same resolution point as Blessed) — same principle on the damage side.
- "On hit": one button per entry of `row.triggerEffects` (`@for (effect of row.triggerEffects; track effect.key)`), active when `effect.trigger() === 'hit'`, `(click)="toggleTriggerEffect(effect, 'hit')"`. `Dispel` and `Grievous Wounds` needed no template change at all to appear here - each is just another `StatEffectType` added to `STAT_EFFECT_TYPES` in `attack-row.model.ts`, and this loop iterates the fixed `triggerEffects` array generically.
...
- "Reset" (`resetEffects(row)`) and "Done" (`close()`) side by side at the bottom of the pop-up (`.dialog__actions`, `flex: 1 1 0` each).

**Content of the Target profile pop-up (`TargetProfileDialog`)** follows the exact same toggle-button pattern:
...
- "Reset" (`resetTargetProfile(t)`) / "Done" (`close()`).
...
`AppMenu` itself owns none of these actions - it only emits `reset`/`openAbout`/`openChangelog`/`openFeedback` outputs and closes itself, mirroring how `TargetPanel`'s cog button emits `openProfile` rather than owning `TargetProfileDialog` (see "Component split" above). "Support me" is the odd one out: a plain `<a>` styled as `.app-menu__item` linking straight to Ko-fi, not an output - there's no state or dialog behind it, just a URL, so wiring it through `OddsCalculator` the way the other four are would've been indirection for its own sake.
...
Telling "first ever load" apart from "returning visitor, been a while" means checking the SAME `HAS_SEEN_ABOUT_KEY` `AboutDialog` uses (imported from `about-dialog.ts`, not duplicated as a second magic string) - but reading it naively inside `ChangelogDialog`'s own `ngAfterViewInit` would race `AboutDialog`'s `ngAfterViewInit` (which is what SETS that key): Angular's `ngAfterViewInit` calls are synchronous and deterministic for a given template, so in practice one of the two siblings always runs first - but *which* one is an implicit function of `<app-about-dialog>`/`<app-changelog-dialog>`'s relative order in `odds-calculator.html`, exactly the kind of accidental coupling that silently breaks on an unrelated future template reorder with no error to catch it. The fix: `ChangelogDialog.ngAfterViewInit` defers its entire check into a `queueMicrotask(...)` callback. Every synchronous `ngAfterViewInit` call for one change-detection pass - including `AboutDialog`'s - completes before any microtask gets a chance to run, so by the time this callback executes, `HAS_SEEN_ABOUT_KEY` reflects its final value for this page load regardless of which dialog's own hook happened to fire first.

This same microtask deferral is also what keeps a plain unit test working: `app.spec.ts`'s `should create the app` test calls `TestBed.createComponent(App)` but never `fixture.detectChanges()`, so `ngAfterViewInit` (and therefore the `queueMicrotask` callback) never actually runs for either dialog in that test - matching `AboutDialog`'s existing behavior there. An earlier version of this code read `localStorage` eagerly (either at module load, or in a class field initializer, which runs during the constructor) specifically to sidestep the sibling-ordering race - both broke that test instead, with `TypeError: localStorage.getItem is not a function`, because `TestBed.createComponent()` DOES run every component's constructor (unlike `ngAfterViewInit`), and this project's Vitest/Node test environment doesn't provide a working `localStorage` at that point (a Node 25 quirk - see the `--localstorage-file` warning in `ng test` output). Deferring the read into `ngAfterViewInit` - which this test never triggers at all - sidesteps that environment gap the same way `AboutDialog` always has, while the microtask still solves the real ordering problem for actual browser page loads.
